...
def setup_whatsapp_integration():
    ...
    if not frappe.db.exists("OD WhatsApp Integration", "OD WhatsApp Integration"):
        ...
        frappe.msgprint(_("WhatsApp Integration document created. Please configure it in the On Desk settings."))
    
    # Create default WhatsApp templates
    create_default_templates()
    
    # Add custom fields to HD Ticket
    from on_desk.on_desk.custom_fields.hd_ticket_whatsapp_fields import setup_all_custom_fields
...
def get_whatsapp_settings():
    ...
    doc = frappe.get_doc("OD WhatsApp Integration", "OD WhatsApp Integration")
    return {
        "enabled": doc.enabled,
...
def send_whatsapp_notification(ticket, template_name=None, message=None):
    ...
    if not phone_number and hasattr(ticket, 'raised_by_phone') and ticket.raised_by_phone:
    ...
    if template_name:
        ...
        return settings.send_message(phone_number, None, template.template_name, template_params)
    elif message:
        # Send text message
