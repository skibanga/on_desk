...
class ODWhatsAppIntegration(Document):
    ...
    def send_message_meta(self, to_number, message, template=None, template_params=None):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.create_message_record(to_number, message, "Outgoing", response_data)
                return response_data
            else:
                frappe.log_error(f"WhatsApp API Error: {response_data}", "WhatsApp Message Error")
                return None
        except Exception as e:
            frappe.log_error(f"WhatsApp API Exception: {str(e)}", "WhatsApp Message Error")
            return None
