...
def get_oauth_providers():
    ...
    try:
        provider_list = frappe.get_all(
            ...
            fields=["name", "provider_name", "icon", "base_url", "client_id", "client_secret"]
        )
        ...
        for provider in provider_list:
            if provider.client_id and provider.client_secret:
                providers.append({
                    ...
                    "auth_url": get_url(f"/api/method/frappe.integrations.oauth2_logins.login_via_{provider.name}"),
...
def get_languages():
    ...
    try:
        lang_list = frappe.get_all("Language", fields=["language_name", "language_code"])
...
def get_provider_text_color(provider_name):
    ...
    dark_background = ["github", "facebook", "twitter", "microsoft", "apple", "linkedin"]
