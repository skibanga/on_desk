...
def get_message_statuses(message_ids):
    ...
    for message in messages:
        try:
            ...
            message_doc = frappe.get_doc("OD Social Media Message", {"message_id": message.message_id})
            ...
            frappe.log_error(f"WhatsApp Message Status Error: {str(e)}", "WhatsApp API Error")
...
def create_conversation(phone_number, contact_name):
    ...
    try:
        ...
        frappe.log_error(f"WhatsApp Create Conversation Error: {str(e)}", "WhatsApp API Error")
        return {"success": False, "error": str(e)}
